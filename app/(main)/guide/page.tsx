...
// 控制是否显示 Demo Day日程 和 路演规则 section
const SHOW_DEMO_DAY_SECTION = false;
const SHOW_ROADSHOW_RULES = false;
...
const SUBMITTER_MAP: Record<string, string> = {
  ...
  '抖音新星捕捞器': '黄风（社区战略组）',
  '问卷数据处理': '樱桃（用户研究一组）',
  // Lightning Coder 校招展示（已移除）
  // Lightning Coder 跨组展示 (DI优秀项目展示)
  '赛博秘书': '阿席（技术战略组）',
  ...
  'Tech Arena网站': '恒宇（社区战略组）+阿瑟（社区战略组）',
...
export default function GuidePage() {
  const timeline = [
    { date: '4月10日', day: '周五', time: '12:00', title: '报名通道开启', highlight: false },
    { date: '4月15日', day: '周三', time: '12:00', title: '报名截止', highlight: true },
    { date: '4月16日', day: '周四', time: '20:00', title: '入选项目公示', highlight: false },
    { date: 'TBD', day: null, time: null, title: 'The Show is ON', highlight: true, tbd: true },
  ];

  const agenda = [
    { time: '13:30 – 13:45', title: '开场白', title2: '& 规则介绍', sub1: null, sub2: null, track: null },
    { time: '13:45 – 16:15', title: 'Lightning Coder', title2: '赛道路演', sub1: '项目展演、投票、评委点评', sub2: null, track: 'lightning_coder' },
    { time: '16:15 – 16:30', title: '茶歇', title2: null, sub1: null, sub2: null, track: null },
    { time: '16:30 – 19:00', title: 'Insighter', title2: '赛道路演', sub1: '项目展演、投票、评委点评', sub2: null, track: 'insighter' },
  ...
  function openSubmit(track: 'lightning_coder' | 'insighter') {
  ...
  return (
    <div className="px-4 md:px-10 pt-4 pb-16 max-w-5xl">
      ...
      <header className="mb-8 pb-2">
        <h2 className="font-headline text-2xl md:text-4xl font-bold text-on-surface">技术演武大会</h2>
      </header>
      ...
      <section className="mb-12">
        ...
        <p className="text-base text-on-surface-variant leading-relaxed">
          第一届小红书社区技术团队（算法/研发）Skills Arena 正式开启；AI 时代已经到来——这里是展示你用 AI 做了什么的舞台
        </p>
      </section>
      ...
      <section className="mb-12">
        ...
        <p className="text-base text-on-surface-variant mb-5">
          每人不限制赛道数量、不限制提交 Skill 数量，欢迎大家踊跃提交。
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

          {/* Lightning Coder */}
          <button
            onClick={() => openSubmit('lightning_coder')}
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-headline text-xl font-bold text-on-surface">Lightning Coder</span>
                </div>
                <span className="text-xs font-semibold text-secondary bg-secondary/10 px-2.5 py-1 rounded-full">≤ 2 人</span>
              </div>
              <div className="space-y-2 mb-3">
                <div>
                  <span className="text-xs font-bold text-secondary bg-secondary/10 px-1.5 py-0.5 rounded">原则</span>
                  <p className="text-sm text-on-surface-variant mt-1 leading-relaxed">极致提高效率，用AI把自己武装成闪电Coder</p>
                </div>
                <div>
            ...
            <div className="p-4">
              ...
              <div className="space-y-2 mb-3">
                <div>
                  ...
                  <p className="text-sm text-on-surface-variant mt-1 leading-relaxed">利用Agent工具发现技术优化点，或提供业务洞察、产品灵感</p>
                </div>
                <div>
                  <span className="text-xs font-bold text-tertiary bg-tertiary/10 px-1.5 py-0.5 rounded">实现形式</span>
                  <p className="text-sm text-on-surface-variant mt-1">AI Skills，鼓励产品化，可以提供你的demo乃至可落地的产品</p>
                </div>
              </div>
      ...
      {/* ── Tech Arena日程 ── */}
      {SHOW_DEMO_DAY_SECTION && (
        <section className="mb-12">
          <SectionTitle>Tech Arena日程</SectionTitle>
            ...
            {agenda.map((item, i) => (
              <div
                ...
                className={`rounded-xl border overflow-hidden ${
                  item.track === 'lightning_coder' ? 'border-secondary/25 bg-secondary/5' :
                  item.track === 'insighter' ? 'border-tertiary/25 bg-tertiary/5' :
                  'border-outline-variant/15 bg-surface-container-low'
                }`}
              >
                <div className={`h-0.5 ${
                  item.track === 'lightning_coder' ? 'bg-secondary' :
                  item.track === 'insighter' ? 'bg-tertiary' : 'bg-transparent'
                }`} />
                <div className="p-4">
                  <p className={`text-xs font-mono font-bold mb-2 ${
                    item.track === 'lightning_coder' ? 'text-secondary/70' :
                    item.track === 'insighter' ? 'text-tertiary/70' : 'text-outline/60'
                  }`}>
                    {item.time}
                  </p>
      ...
      {SHOW_ROADSHOW_RULES && (
        <section className="mb-12">
          ...
          <p className="text-sm text-on-surface-variant mb-4">
            每个 Skill <span className="font-semibold text-on-surface">5 分钟展演 + 2 分钟 QA</span>
          </p>
          ...
          {timeline.map((item, i) => (
            <div key={i} className="flex-1 flex flex-row md:flex-col items-center md:items-center gap-3 md:gap-0 py-2 md:py-0">
              ...
              <div className="text-left md:text-center px-1">
                ...
                {item.tbd ? (
                  <p className="text-xs text-on-surface-variant/40">TBD</p>
                ) : (
                  <>
                    {item.day && <p className="text-xs text-on-surface-variant">{item.date} {item.day}</p>}
                    {item.time && <p className={`text-xs font-mono ${item.highlight ? 'text-secondary/80' : 'text-on-surface-variant/50'}`}>@ {item.time}</p>}
                  </>
                )}
              </div>
            </div>
